...
static uint16_t package_counter_hold = 0;
/**
...
void ep1_out_handler(uint8_t *buf, uint16_t len) {
    ...
    package_counter_hold = 0;
    package_counter = 0;
    for(uint8_t i;i<len;i++){
        package_counter_hold = package_counter_hold | (uint16_t)(*buf)<<(i*8);
        buf++;
    }
    usb_start_transfer(ep, test_data, 64);
    package_counter++;
...
void ep2_in_handler(uint8_t *buf, uint16_t len) {
    ...
    if(package_counter <= package_counter_hold){
        usb_start_transfer(ep, test_data, 64);
        package_counter++;
        }
    else
